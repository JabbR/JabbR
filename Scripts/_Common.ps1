...
if($programFiles -eq ${env:ProgramFiles(x86)})
{
  $programFiles = Join-Path (Split-Path $programFiles) "Program Files"
}

$AzureToolsRoot = "$programFiles\Microsoft SDKs\Windows Azure\"
$UseEmulator = $env:JABBR_USE_EMULATOR -eq $true