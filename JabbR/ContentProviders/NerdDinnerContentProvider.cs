...
using JabbR.NerdDinnersServiceDataContext;
    ...
    public class NerdDinnerContentProvider : CollapsibleContentProvider
    {
        ...
        protected override string GetCollapsibleContent(HttpWebResponse response)
        {
            ...
            if (!String.IsNullOrEmpty(strDinnerId))
            {
                ...
                NerdDinners dataService = new NerdDinners(new Uri(
                   "http://nerddinner.com/Services/OData.svc/"));

                var dinner = (from d in dataService.Dinners
                ...
                if (null != dinner)
                {
                    return String.Format(_nerdDinnerContentFormat,
                        ...
                        dinner.Title,
                        dinner.EventDate.Date.ToLongDateString(),
                        dinner.EventDate.ToLongTimeString(),
                        dinner.Address,
                        dinner.Description,
                        dinner.DinnerID));
                }
        ...
        {
            return _vimeoIdRegex.Match(responseUri.AbsoluteUri)
                                .Groups
                                .Cast<Group>()
                                .Skip(1)
                                .Select(g => g.Value)
